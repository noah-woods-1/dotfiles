...
function print_prompt_head {
    local left_prompt="\
%{$red%}┌─╼\
%{$white_bold%}$(get_usr_name)\
%{$blue_bold%}@\
%{$cyan_bold%}$(get_box_name)%{$reset_color%}%{$red_bold%} [\
%{$yellow_bold%}$(get_current_dir)%{$reset_color%}%{$red_bold%}]%{$reset_color%}\
$(get_git_prompt) "
...
function get_prompt_indicator {
    if [[ $? -eq 0 ]]; then
        echo "%{$red%}└────╼%{$yellow_bold%}$zeta %{$reset_color%}"
    else
        echo "%{$red%}└────╼%%{$red_bold%}$PROMPT %{$reset_color%}"
    fi
}
